...
def extract_poses(
    ...
    if locref is not None:
        ...
        locref = locref.reshape(batch_size, num_keypoints, 2, h, w).transpose(2, 3).reshape(batch_size, num_keypoints, h, w, 2)

        max_vals, max_indices = scmap.view(batch_size, num_keypoints, -1).max(dim=-1)  # Get max values and their indices
        x = max_indices % w
        y = max_indices // w

        locref_x = locref[np.arange(batch_size)[:, None], np.arange(num_keypoints)[None], y, x, 0] * locref_stdev
        locref_y = locref[np.arange(batch_size)[:, None], np.arange(num_keypoints)[None], y, x, 1] * locref_stdev

        x = x.view(batch_size, num_keypoints, 1) * stride + stride / 2 + locref_x.view(batch_size, num_keypoints, 1)
        y = y.view(batch_size, num_keypoints, 1) * stride + stride / 2 + locref_y.view(batch_size, num_keypoints, 1)
        likelihood = max_vals.view(batch_size, num_keypoints, 1)

        pred = torch.cat((x, y, likelihood), dim=2).squeeze(0).numpy() # Assuming batch_size=1 for live
